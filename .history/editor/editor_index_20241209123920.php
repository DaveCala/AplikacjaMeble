...
#notificationPanel {
  ...
  border: 1px solid #2d3748; /* Dodanie granicy wokół panelu */
  max-height: 400px; /* Wysokość panelu, wyświetlanie 3 powiadomień na raz */
  display: flex;
...
#notificationPanel ul {
  padding: 0;
  flex-grow: 1; /* Sprawia, że lista rośnie, ale pozostawia miejsce na dolny pasek */
  overflow-y: auto; /* Scrollowanie, jeśli jest więcej powiadomień niż przestrzeni */
}

#notificationPanel li {
  padding: 12px;
  border-top: 1px solid #2d3748; /* Granica między elementami */
  transition: background-color 0.3s, border-color 0.3s; /* Dodanie efektu przejścia */
}

#notificationPanel li:hover {
  background-color: #2b6cb0; /* Niebieski kolor tła na hover */
  border-color: #3182ce; /* Niebieska granica na hover */
  cursor: pointer;
}

#notificationPanel .p-4 {
  border-bottom: 1px solid #2d3748; /* Granica pomiędzy nagłówkiem a listą */
}

#notificationPanel h3 {
  color: #e2e8f0; /* Jasny kolor tekstu dla nagłówka */
}

/* Pseudo zakończenie listy na stałe na dole */
#notificationPanel > div {
  border-top: 2px solid #2d3748; /* Granica górna */
  border-radius: 0 0 8px 8px; /* Zaokrąglony dolny róg */
  background-color: #1a202c; /* Tło końcowego elementu */
  color: #4a5568; /* Subtelny kolor tekstu */
}
...
<div id="notificationPanel" class="hidden absolute top-14 right-0 bg-gray-900 shadow-lg rounded-md w-80 max-h-96 overflow-y-auto z-50">
  ...
  <div class="border-t-2 border-gray-600 rounded-b-md p-4 bg-gray-800">
