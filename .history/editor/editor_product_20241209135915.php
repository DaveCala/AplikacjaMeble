...
document.querySelectorAll('.toggle-details').forEach((button) => {
    button.addEventListener('click', function () {
        ...
        // Wyszukaj element wariacji na podstawie ID (opcjonalnie, jeśli chcesz więcej danych)
        const variationElement = document.querySelector(`input[data-variation-id="${variationId}"]`).closest('.bg-gray-900');

        // Wyciągnij dane z elementu (np. tytuł i EAN)
        ...
        // Wyświetl dane w konsoli lub zaktualizuj interfejs
        console.log(`ID: ${variationId}, Tytuł: ${title}, EAN: ${ean}`);
        alert(`ID: ${variationId}\nTytuł: ${title}\nEAN: ${ean}`);
    });
});
