...
<div id="add-product-form" class="hidden bg-gray-900 p-6 rounded-lg shadow-lg mb-6">
  ...
  <form id="add-product" method="POST" enctype="multipart/form-data" action="add_product.php">
    ...
    <div class="mb-4">
      ...
      <div>
        <label class="inline-flex items-center">
          <input
            type="radio"
            name="is_variation"
            value="true"  <!-- Wartość 'true' oznacza "Produkt z wariacjami" -->
      ...
      <div>
        <label class="inline-flex items-center">
          <input
            type="radio"
            name="is_variation"
            value="false" <!-- Wartość 'false' oznacza "Produkt bez wariacjami" -->
            class="form-radio text-green-500"
            required
            onclick="toggleVariationForm()"
          />
      ...
      <!-- Formularz dodatkowych danych, który pojawi się tylko, gdy "Produkt z wariacjami" -->
...
document.querySelector('input[placeholder="Wyszukaj meble"]').addEventListener('input', function () {
  ...
  function toggleVariationForm() {
    ...
    if (isVariation === 'true') {
      // Pokazuje pola dla produktów z wariacjami
      variationFields.style.display = 'block';
    } else {
      // Ukrywa pola dla produktów bez wariacji
