...
window.onload = function() {
  ...
  const productId = urlParams.get('id');  // Pobieramy ID z URL
  
  // Ustawiamy wartość w ukrytym polu formularza
  if (productId) {
    document.getElementById('product-id').value = productId;
  } else {
    alert('Brak ID produktu w URL!');
  }
  ...
  document.getElementById('add-variation').addEventListener('submit', function(e) {
    ...
    .then(data => {
      if (data.success) {
        ...
        form.reset();
        loadVariationList();  // Ładowanie zaktualizowanej listy wariacji
      } else {
  ...
  function loadVariationList() {
    ...
    fetch('fetch_variations.php?product_id=' + productId)
      .then(response => response.json())
      .then(data => {
        const variationsContainer = document.getElementById('variations-list');
        variationsContainer.innerHTML = '';  // Czyścimy listę przed załadowaniem nowych danych

        // Dodanie każdej wariacji do listy
        data.forEach(variation => {
          const item = document.createElement('div');
          item.className = 'variation-item';
          item.innerHTML = `
            <p><strong>Tytuł:</strong> ${variation.title}</p>
            <img src="../img/variations/${variation.image}" alt="${variation.title}" width="100">
            <p><strong>EAN:</strong> ${variation.ean}</p>
          `;
          variationsContainer.appendChild(item);
        });
      })
      .catch(error => console.error('Błąd podczas ładowania wariacji:', error));
  }
};
