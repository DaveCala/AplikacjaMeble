...
<form id="edit-product-form" enctype="multipart/form-data">
  ...
  <!-- Pole do wyświetlania i edytowania zdjęcia -->
  <div class="mb-4">
    <label for="edit-product-image" class="block mb-2 text-sm">Aktualne zdjęcie:</label>
    <div class="mb-2">
      <?php if (!empty($product['image'])): ?>
        <img src="<?php echo htmlspecialchars($product['image']); ?>" alt="Produkt" class="w-32 h-32 object-cover rounded-lg product-image">
        <p class="text-sm text-gray-400 mt-2">Aby zmienić zdjęcie, wybierz nowe.</p>
      <?php else: ?>
        <p class="text-sm text-gray-400 mt-2">Brak zdjęcia produktu.</p>
      <?php endif; ?>
    </div>
    <label for="edit-product-image" class="block mb-2 text-sm">Zmień zdjęcie (opcjonalnie):</label>
  ...
  <button type="submit" class="py-3 px-6 bg-green-600 rounded-lg text-white text-lg hover:bg-green-500">
    Zapisz zmiany
  </button>
</form>
  ...
  document.getElementById('edit-product-form').addEventListener('submit', (e) => {
    e.preventDefault(); // Zapobiega domyślnej akcji formularza (przeładowanie strony)
    ...
    .then(data => {
        if (data.success) {
            ...
            // Można tutaj zaktualizować widok na stronie bez przeładowania
            document.querySelector('#edit-product-title').value = data.updatedTitle;
            document.querySelector('#edit-product-category').value = data.updatedCategory;
            document.querySelector('#edit-product-description').value = data.updatedDescription;
            if (data.updatedImage) {
                document.querySelector('.product-image').src = `../img/${data.updatedImage}`;
            }
        } else {
