...
<div class="container mx-auto p-6">
    ...
    $productId = 1; // Przykładowe ID produktu
    $query = "SELECT * FROM products WHERE id = ?";
    $stmt = $pdo->prepare($query);
    $stmt->execute([$productId]);
    $product = $stmt->fetch(PDO::FETCH_ASSOC);

    // Filtruj dane produktu, aby pozostawić tylko kolumny z wartościami
    $filteredColumns = array_filter($product, function ($value) {
        return $value !== null && $value !== ''; // Usuń puste wartości
    });
    ?>
    ...
    <form id="edit-product-form" enctype="multipart/form-data">
        ...
        <?php foreach ($filteredColumns as $column => $value): ?>
            <?php if ($column !== 'image'): // Pomijamy zdjęcie ?>
                <div class="mb-4">
