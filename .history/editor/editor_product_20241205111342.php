...
<div id="add-variation-form" class="hidden bg-gray-900 p-6 rounded-lg shadow-lg mb-6">
  ...
  <form id="add-variation" method="POST" enctype="multipart/form-data">
    <div class="mb-4 text-white">
      <label for="product-id" class="block mb-2 text-sm">ID Produktu:</label>
      <input type="number" id="product-id" name="product_id" class="w-full p-3 rounded-lg bg-gray-700 text-white" required>
    </div>
    <div class="mb-4 text-white">
...
document.getElementById('add-variation').addEventListener('submit', function (e) {
  ...
  .then(data => {
    if (data.success) {
      alert('Wariacja została dodana pomyślnie!');
      location.reload(); // Odświeżenie strony lub dynamiczne dodanie wariacji do listy
    } else {
      alert('Błąd: ' + data.message);
    }
  })
  .catch(error => {
    console.error('Błąd:', error);
    alert('Wystąpił problem podczas dodawania wariacji.');
  });
});
