...
form.addEventListener('submit', function (event) {
    ...
    const formData = new FormData(form);
    formData.append('product_id', document.getElementById('product-id').value); // Dodaj ID produktu

    fetch('add_variation.php', { // Upewnij się, że ścieżka jest poprawna
        method: 'POST',
        body: formData,
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                alert('Nie udało się dodać wariacji: ' + data.error);
            }
        })
        .catch(error => {
            console.error('Błąd:', error);
