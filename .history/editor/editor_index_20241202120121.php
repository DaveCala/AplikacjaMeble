...
<div id="add-product-form" class="hidden bg-gray-900 p-6 rounded-lg shadow-lg mb-6">
  ...
  <script>
    // Toggle formularza
    document.getElementById('toggle-add-form').addEventListener('click', function (event) {
  event.preventDefault(); // Zapobiegamy domyślnej akcji (czyli wysyłaniu formularza)
  ...
  // Sprawdzamy, czy tytuł i kategoria nie są puste
  const title = formData.get('title');
  const category = formData.get('category');
  if (!title || !category) {
    alert('Wszystkie pola muszą być wypełnione.');
    return; // Zatrzymujemy dalsze działanie, jeśli są puste pola
  }
