...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (toggleButton && form) {
    ...
    console.error('Nie znaleziono elementów przycisku lub formularza.');
  }
});
...
.then(response => response.text())  // Zmieniamy na response.text() by zobaczyć pełną odpowiedź
.then(responseText => {
  console.log('Odpowiedź serwera:', responseText);  // Zobacz, co dokładnie przychodzi w odpowiedzi
  try {
    const data = JSON.parse(responseText);  // Parsowanie JSON ręcznie
    if (data.success) {
    ...
    alert('Błąd parsowania odpowiedzi serwera.');
  }
})
.catch(error => {
  console.error('Błąd:', error);
