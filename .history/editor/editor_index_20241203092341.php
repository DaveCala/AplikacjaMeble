...
<nav class="bg-gray-900 navbar w-full py-6 text-white flex justify-between items-center">
  ...
  <div class="flex items-center space-x-4 mr-6">
    <a href="../login/logout.php" class="bg-gray-900 border border-red-500 text-red-500 py-2 px-4 rounded-lg hover:bg-red-800 hover:text-white text-sm">Wyloguj się</a>
    <?php if ($_SESSION['user']['role'] === 'admin') : ?>
      <a href="../admin/users.php" class="bg-gray-900 border border-white text-white py-2 px-4 rounded-lg hover:bg-gray-700 text-sm">Zarządzaj użytkownikami</a>
    <?php endif; ?>
  </div>
</nav>
...
<div class="mx-auto my-10 w-3/4 md:w-1/2">
  <input type="text" placeholder="Wyszukaj meble" class="w-full py-2 px-4 text-lg rounded-lg bg-gray-700 text-white">
</div>
...
<div class="flex justify-between items-center mt-10 mb-4 mx-6">
  ...
  <div class="flex items-center space-x-4">
    <div id="delete-button-container" class="hidden">
      <button id="delete-selected" class="bg-gray-800 text-red-500 border border-red-500 py-2 px-4 rounded-lg hover:bg-red-700 hover:text-white">Usuń zaznaczone</button>
    </div>
    <button id="toggle-add-form" class="py-2 px-4 bg-gray-800 rounded-lg border border-green-500 text-green-500 text-lg hover:bg-green-500 hover:text-white">Dodaj</button>
  </div>
</div>
...
<div id="add-product-form" class="hidden bg-gray-900 p-6 rounded-lg shadow-lg mb-6">
  ...
  <form id="add-product" method="POST" enctype="multipart/form-data" action="add_product.php">
    ...
    <div class="mb-4 text-white">
      ...
      <input type="file" id="product-image" name="image" class="block w-full text-sm text-gray-300 bg-gray-700">
    </div>
    <div class="flex justify-center mb-6">
      <button type="submit" class="py-2 px-4 bg-green-600 rounded-lg text-white text-lg hover:bg-green-500">Dodaj</button>
    </div>
  </form>
</div>
...
<script>
// Funkcja do inicjalizacji checkboxów
function initProductCheckboxes() {
  document.querySelectorAll('.product-checkbox').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
      ...
      document.getElementById('delete-button-container').classList.toggle('hidden', !anyChecked);
    });
  });
}
...
document.getElementById('delete-selected').addEventListener('click', function() {
  ...
  if (selectedIds.length > 0) {
    ...
    .then(data => {
      if (data.success) {
        alert('Produkty zostały usunięte.');
        loadProductList();
      } else {
...
// Inicjalizacja checkboxów po załadowaniu strony
initProductCheckboxes();
</script>
