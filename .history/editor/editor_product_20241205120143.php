...
<body class="bg-gray-800 text-white">
  ...
  <!-- Formularz dodawania nowej wariacji -->
<div id="add-variation-form" class="bg-gray-900 p-6 rounded-lg shadow-lg mb-6">
    <h2 class="text-2xl text-white mb-4">Dodaj nową wariację</h2>
    <form id="add-variation" enctype="multipart/form-data">
        <input type="hidden" name="product_id" value="<?php echo htmlspecialchars($productId); ?>">
        <div class="mb-4">
            <label for="variation-title" class="block mb-2 text-sm text-white">Tytuł wariacji:</label>
            <input type="text" id="variation-title" name="title"
                   class="w-full p-3 rounded-lg bg-gray-700 text-white" required>
        </div>
        <div class="mb-4">
            <label for="variation-ean" class="block mb-2 text-sm text-white">EAN:</label>
            <input type="text" id="variation-ean" name="ean"
                   class="w-full p-3 rounded-lg bg-gray-700 text-white" required>
        </div>
        <div class="mb-4">
            <label for="variation-image" class="block mb-2 text-sm text-white">Zdjęcie główne:</label>
            <input type="file" id="variation-image" name="main_image"
                   class="block w-full text-sm text-gray-300 bg-gray-700 rounded-lg">
        </div>
        <button type="button" id="add-variation-submit"
...
document.getElementById('add-variation-submit').addEventListener('click', () => {
    const form = document.getElementById('add-variation');
    const formData = new FormData(form);
...
.then(data => {
    if (data.success) {
        ...
        const variationsList = document.querySelector('.grid');
        const newVariation = document.createElement('div');
        newVariation.className = 'bg-gray-900 p-4 border border-gray-700 rounded-lg shadow-md';

        newVariation.innerHTML = `
            <h3 class="text-white text-lg font-semibold mb-2">${formData.get('title')}</h3>
            <p class="text-gray-400 mb-2">EAN: ${formData.get('ean')}</p>
            <div class="mb-4">
                <img src="../img/${data.image}" alt="${formData.get('title')}" 
                     class="h-32 w-auto object-contain mx-auto rounded-lg">
            </div>
        `;
