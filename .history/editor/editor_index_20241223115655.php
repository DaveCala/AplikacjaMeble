...
function addDynamicField() {
  ...
  newField.required = true; // Oznacz jako wymagane
  newField.className = "w-full p-3 rounded-lg bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500";
  container.appendChild(newField);
}

document.getElementById("add-product").addEventListener("submit", function (event) {
  const form = event.target;
  const inputs = form.querySelectorAll("input, textarea, select"); // Pobierz wszystkie pola
  let isValid = true;
  ...
  inputs.forEach((input) => {
    ...
    if (input.hasAttribute("required") && !input.value.trim()) {
      isValid = false;
      input.classList.add("border-red-500"); // Dodaj klasę błędu, np. czerwony obramowanie
